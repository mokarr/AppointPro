...
export default async function BookingSuccessPage({
    ...
    // Send confirmation email after successful booking fetch
    if (booking.customerEmail) {
        const headersList = await headers();
        const host = headersList.get('host');
        const protocol = process.env.NODE_ENV === 'production' ? 'https' : 'http';
        const bookingLink = `${protocol}://${host}/book/confirmation/success?bookingId=${booking.id}`;
        console.log('bookingLink', bookingLink);
        const bookingData: BookingConfirmationInput = {
            bookingId: booking.id,
            customerName: booking.customerName || 'Klant',
