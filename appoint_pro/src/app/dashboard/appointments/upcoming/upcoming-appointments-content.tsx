...
import AppointmentForm from '@/app/dashboard/appointments/timetable/appointment-form';
...
export default function UpcomingAppointmentsContent({ _user, _organization, _locations }: UpcomingAppointmentsContentProps) {
    ...
    const handleCreateAppointmentClick = () => {
        if (!selectedLocation || facilities.length === 0) {
            toast({
                ...
                description: getTranslation("common.selectLocationFirst"),
                variant: "destructive"
            });
            return;
        }
    ...
    const handleAppointmentSubmit = async (values: AppointmentFormValues) => {
        try {
            ...
            toast({
                ...
                description: getTranslation("common.appointmentCreated"),
            });
            ...
            toast({
                title: getTranslation("common.error"),
                description: getTranslation("common.appointmentCreateFailed"),
                variant: "destructive"
            });
    ...
    return (
        <DashboardLayout>
            <DashboardHeader
                heading={getTranslation('common.upcomingAppointments')}
                description={getTranslation('common.header.upcomingAppointments.description')}
                action={
                            ...
                            <div className="space-y-8">
                                {facilities.length === 0 ? (
                                    <p className="text-gray-500 dark:text-gray-400 text-center py-8">
                                        {getTranslation('common.noFacilitiesFound')}
                                    </p>
                                ) : (
                                            ...
                                            <div key={facilityId} className="border rounded-lg overflow-hidden">
                                                ...
                                                {facilityAppointments.length === 0 ? (
                                                    <div className="p-6 text-center">
                                                        <p className="text-gray-500 dark:text-gray-400">
                                                            {getTranslation('common.noAppointmentsFound')}
                                                        </p>
                                                    </div>
                                                ) : (
